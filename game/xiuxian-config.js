...
3. logicChain（推理链条）：按步骤展示你的决策过程
   ...
   - 步骤5：【剧情设计】本轮如何制造惊喜？（转折/伏笔/悬念，至少选一个）
   - 步骤6：设计合理的后续选项
    ...
    return `
        ...
        <div class="panel">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <h2 style="margin: 0;">角色状态</h2>
                <button onclick="openVariableEditor()" style="padding: 5px 12px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 12px;">编辑变量</button>
            </div>
            ...
            <div class="status-section">
                ...
                <div class="status-item">
                    ...
                    <span class="status-value" id="charPersonalityDisplay">-</span>
                </div>
                <div class="status-item">
                    <span class="status-label">外貌：</span>
                    <span class="status-value" id="charAppearance">-</span>
                </div>
            </div>
...
function renderStatusPanel(vars) {
    ...
    if (charName) charName.textContent = variables.name || '未知';

    const charAge = document.getElementById('charAge');
    if (charAge) charAge.textContent = variables.age || '-';

    const charGender = document.getElementById('charGender');
    if (charGender) charGender.textContent = variables.gender || '-';

    const charIdentity = document.getElementById('charIdentity');
    if (charIdentity) charIdentity.textContent = variables.identity || '-';

    const charRealm = document.getElementById('charRealm');
    if (charRealm) charRealm.textContent = variables.realm || '-';

    const charLocation = document.getElementById('charLocation');
    if (charLocation) charLocation.textContent = variables.location || '-';

    const charTalents = document.getElementById('charTalents');
    if (charTalents) charTalents.textContent = variables.talents && variables.talents.length > 0 ? variables.talents.join('、') : '-';

    // 性格和外貌
    const charPersonalityDisplay = document.getElementById('charPersonalityDisplay');
    if (charPersonalityDisplay) charPersonalityDisplay.textContent = variables.personality || '-';

    const charAppearance = document.getElementById('charAppearance');
    if (charAppearance) charAppearance.textContent = variables.appearance || '-';
